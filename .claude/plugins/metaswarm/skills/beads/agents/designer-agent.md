...
ls src/api/ (example path)routes/ (example path)

# Find similar service interfaces
grep -r "interface.*Service" src/lib/ (example path)services/ (example path) --include="*.ts" | head -20

# Check component naming patterns
ls src/components/ (example path)

# Review existing Zod schemas
ls src/lib/ (example path)schemas/
```
